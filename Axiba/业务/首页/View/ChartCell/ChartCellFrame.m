...
-(void)setChartMessage:(ChartMessage *)chartMessage
{
    ...
    self.labelNameRect = CGRectMake(iconX+iconWidth + 8, iconY + 6, 150, 16);
    self.labelTimeRect = CGRectMake(kTPScreenWidth - 96, iconY + 6, 80, 16);
    
    if(![chartMessage.otherInfo isValid])
    {
        chartMessage.otherInfo = @"";
    }
    
    CGFloat contentX    = CGRectGetMaxX(self.iconRect)+kIconMarginX;
    CGFloat contentY    = iconY + 10;
    NSDictionary *attributes = @{NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:13]};
    CGSize contentSize=[[NSString stringWithFormat:@"%@%@",chartMessage.otherInfo,chartMessage.content]  boundingRectWithSize:CGSizeMake(kTPScreenWidth-110, MAXFLOAT) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributes context:nil].size;
