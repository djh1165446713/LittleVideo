...
- (id)initWithNamespace:(NSString *)ns {
    if ((self = [super init])) {
        ...
        dispatch_sync(_ioQueue, ^{
            wSelf.fileManager = [NSFileManager new];
        });
...
- (void) storeResponse: (id)response forUrl: (NSString *) url byParam:(NSDictionary *)params toDisk: (BOOL)toDisk {
    // - currently only supports for the dictionary objects, require auto detection for different types and convert to data for saving
    // first convert the response to data
    
    if (![response isKindOfClass:[NSDictionary class]]) {
        NSAssert(NO, @"目前只支持NSDictionary保存缓存");
        return;
    }
...
- (void)clearDiskOnCompletion:(XBHttpCacheNoParamsBlock)completion
{
    ...
    dispatch_async(self.ioQueue, ^{
        [wSelf.fileManager removeItemAtPath:wSelf.diskCachePath error:nil];
        [wSelf.fileManager createDirectoryAtPath:wSelf.diskCachePath
...
- (void)cleanDiskWithCompletionBlock:(XBHttpCacheNoParamsBlock)completionBlock {
    ...
    dispatch_async(self.ioQueue, ^{
        ...
        NSDirectoryEnumerator *fileEnumerator = [sSelf.fileManager enumeratorAtURL:diskCacheURL
        ...
        for (NSURL *fileURL in urlsToDelete) {
            [sSelf.fileManager removeItemAtURL:fileURL error:nil];
        }
        ...
        if (sSelf.maxCacheSize > 0 && currentCacheSize > sSelf.maxCacheSize) {
            ...
            for (NSURL *fileURL in sortedFiles) {
                if ([sSelf.fileManager removeItemAtURL:fileURL error:nil]) {
...
- (NSUInteger)getSize {
    ...
    dispatch_sync(self.ioQueue, ^{
        NSDirectoryEnumerator *fileEnumerator = [sSelf.fileManager enumeratorAtPath:sSelf.diskCachePath];
